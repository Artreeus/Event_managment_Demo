...
const demoCategories = [
  ...
  { name: 'Photography', description: 'Professional photography services for all occasions', icon: '📸', image: 'https://images.unsplash.com/photo-1542038784456-1ea8e935640e?w=800', displayOrder: 3 },
...
export async function POST() {
  try {
    ...
    // ── Create users: hash passwords manually, insert via raw collection
    //    to bypass any cached pre-save hook state across hot reloads ─────────
    const now = new Date();
    ...
    const insertResult = await User.collection.insertMany(usersToInsert);
    const createdUsers = usersToInsert.map((u, i) => ({
      ...u,
